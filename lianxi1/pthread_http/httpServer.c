...
//（1）套接字结构体
    ...
    addr.sin_port = htons(port);//主机序列转换为网络序列
    addr.sin_addr.s_addr = INADDR_ANY;//可以绑定任意ip  命令行不用输入ip
//(2)创建套接字
...
//（5）监听套接字  客户端是否发来请求，listen为监听状态，客户端最多有五个客户端等待连接
...
// 3# 请求处理函数（重点）
//arg就相当于之前已经连接的客1户端里包含的请求报文
static void* handle_request(void* arg)
{
    int sock = (int)arg;
    char line[MAX_SIZE];//缓冲区接收报文
    int status_code = 200; // 状态码

//line缓冲区 sizeof（line)缓冲区大小
    // getline函数一行行提取请求信息
...
//资源路径 	  光有/ 则访问的主页 webroot/index.html
//创建数组保存路径
...
//st 保存文件的状态
    ...
    else
    {
        ...
        {
            strcat(resource_path, "index.html"); // 把目录的首页拼接到资源路径后
        }
        // 如果请求的资源具有可执行权限
        else if(
                    (st.st_mode & S_IXUSR) ||
...
//##可变参数  ./程序  port      argc：参数   argv[]：存参数的数组
int main(int argc, char* argv[])
{
    ...
    while(1)
    {
		...
		//获取监听套接字  分配文件描述符给客户端 进行通信
		//客户端发送的信息都包含在connect_fd这个描述符里边
//（6）接收套接字
...
//调试程序时看IP和端口是什么
        ...
        pthread_t tid = 0;
//handle_request处理请求(主要)--------浏览器发来的请求进行处理
